...
export function createBridgeReplyDispatcher(params: {
  ...
  const startStreaming = () => {
    if (streamingStarted || !creds) return;
    streamingStarted = true;
    streamingStartPromise = (async () => {
      streaming = new FeishuStreamingSession(
        client,
        creds,
        (msg) => log(`feishu: streaming: ${msg}`),
      );
      try {
  ...
  return {
    onPartialText(text: string) {
      ...
      if (!streamingStarted) startStreaming();
    ...
    async onComplete(text: string, segments: string[], toolCalls: string[], durationMs: number) {
