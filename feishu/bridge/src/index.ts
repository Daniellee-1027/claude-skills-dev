...
function parseArgs(argv: string[]): { cwd: string; config: string } {
  let cwd = process.cwd();
  let config = "";
...
async function main() {
  ...
  const log = (...msgs: any[]) => console.log(`[${new Date().toISOString()}]`, ...msgs);
  const error = (...msgs: any[]) => console.error(`[${new Date().toISOString()}]`, ...msgs);
