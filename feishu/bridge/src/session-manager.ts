...
function wrapHandler(handler: (params: any) => Promise<string>) {
  return async (args: any) => {
    try {
      const result = await handler(args);
      return { content: [{ type: "text" as const, text: result }] };
    } catch (err: any) {
      return { content: [{ type: "text" as const, text: JSON.stringify({ error: err.message ?? String(err) }) }] };
    }
  };
}
 ...
 */
function buildSdkMcpTools(toolHandlers: Map<string, { handler: (params: any) => Promise<string>; description: string; inputSchema: any }>) {
  const tools = [];
  ...
  if (handlers.feishu_wiki) {
    tools.push(tool(
      "feishu_wiki",
      "Feishu knowledge base operations. Actions: spaces (list all), nodes (browse), get (read node), create, move (within wiki), rename, delete, import_to_wiki (import cloud-space doc/bitable INTO wiki). No search — must browse via spaces → nodes → get.",
      {
  ...
  if (handlers.feishu_bitable_get_meta) {
    tools.push(tool(
      ...
      { url: z.string().describe("Bitable URL (wiki or base format)") },
      wrapHandler(handlers.feishu_bitable_get_meta.handler),
    ));
  }
  ...
  if (handlers.feishu_bitable_list_records) {
    tools.push(tool(
      "feishu_bitable_list_records",
      "List records (rows) from a Bitable table with pagination",
      {
        app_token: z.string().describe("Bitable app token"),
        table_id: z.string().describe("Table ID"),
  ...
  if (handlers.feishu_bitable_get_record) {
    tools.push(tool(
      "feishu_bitable_get_record",
      "Get a single record by ID from a Bitable table",
      {
        app_token: z.string().describe("Bitable app token"),
        table_id: z.string().describe("Table ID"),
  ...
  if (handlers.feishu_bitable_create_record) {
    tools.push(tool(
      "feishu_bitable_create_record",
      "Create a new record (row) in a Bitable table. Use field names as keys in fields object.",
      {
        app_token: z.string().describe("Bitable app token"),
        table_id: z.string().describe("Table ID"),
  ...
  if (handlers.feishu_bitable_update_record) {
    tools.push(tool(
      "feishu_bitable_update_record",
      "Update an existing record in a Bitable table. Only specified fields are modified.",
      {
        app_token: z.string().describe("Bitable app token"),
        table_id: z.string().describe("Table ID"),
        record_id: z.string().describe("Record ID"),
  ...
  if (handlers.feishu_bitable_create_field) {
    tools.push(tool(
      "feishu_bitable_create_field",
      "Create a new field (column) in a Bitable table. field_type: 1=Text, 2=Number, 3=SingleSelect, 4=MultiSelect, 5=DateTime, 7=Checkbox, 15=URL",
      {
        app_token: z.string().describe("Bitable app token"),
        table_id: z.string().describe("Table ID"),
  ...
  if (handlers.feishu_bitable_import_csv) {
    tools.push(tool(
      "feishu_bitable_import_csv",
      "Import a local CSV file into a new Feishu Bitable. Auto-infers field types, creates table, batch imports all rows. Optionally moves to wiki space.",
      {
  ...
  if (handlers.feishu_bitable_batch_create_records) {
    tools.push(tool(
      "feishu_bitable_batch_create_records",
      "Batch create records (rows) in a Bitable table. Max 500 per call. Each item in records is a {field_name: value} object.",
      {
        app_token: z.string().describe("Bitable app token"),
        table_id: z.string().describe("Table ID"),
  ...
  if (handlers.feishu_bitable_batch_delete_records) {
    tools.push(tool(
      "feishu_bitable_batch_delete_records",
      "Batch delete records from a Bitable table",
      {
        app_token: z.string().describe("Bitable app token"),
        table_id: z.string().describe("Table ID"),
  ...
  if (handlers.feishu_bitable_rename_field) {
    tools.push(tool(
      "feishu_bitable_rename_field",
      "Rename a field (column) in a Bitable table",
      {
        app_token: z.string().describe("Bitable app token"),
        table_id: z.string().describe("Table ID"),
    ...
    handleSlashCommand(sessionKey: string, command: string): SlashCommandResult {
      ...
      switch (lowerCmd) {
        ...
        case "/model": {
          ...
          if (args.length === 0) {
            // Show current model
            const aliases = Object.entries(MODEL_ALIASES).map(([alias, id]) =>
              id === currentModel ? `**${alias}** ✅` : alias
            ).join(" / ");
