...
// ============ Exported Actions ============
export async function readDoc(client, docToken) {
...
export async function writeDoc(client, docToken, markdown) {
...
export async function appendDoc(client, docToken, markdown) {
    const { blocks, firstLevelBlockIds } = await convertMarkdown(client, markdown);
    if (blocks.length === 0)
...
export async function updateBlock(client, docToken, blockId, content) {
...
export async function deleteBlock(client, docToken, blockId) {
    const blockInfo = await client.docx.documentBlock.get({ path: { document_id: docToken, block_id: blockId } });
    if (blockInfo.code !== 0)
        throw new Error(blockInfo.msg);
...
export async function listBlocks(client, docToken) {
    const res = await client.docx.documentBlock.list({ path: { document_id: docToken } });
    if (res.code !== 0)
        throw new Error(res.msg);
    return { blocks: res.data?.items ?? [] };
}
export async function getBlock(client, docToken, blockId) {
