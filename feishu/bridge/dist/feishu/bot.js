...
import { escapeRegExp, extractMentionTargets, extractMessageBody, isMentionForwardRequest, } from "./mention.js";
...
function recordHistory(chatId, sender, body) {
...
function getAndClearHistory(chatId) {
...
export async function handleFeishuMessage(params) {
    ...
    if (isGroup) {
        ...
        if (config.feishu.dmPolicy === "allowlist") {
            const allowed = (config.feishu.allowFrom ?? []).some((id) => id === "*" ||
    ...
    try {
        ...
        await sessionManager.handleMessage({
            ...
            onComplete: (text, segments, toolCalls, durationMs) => replyDispatcher.onComplete(text, segments, toolCalls, durationMs),
            onError: async (err) => {
